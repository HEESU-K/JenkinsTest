pipeline {
    ...
    environment {
        DOCKER_IMAGE = 'kh55s/jenkinstest'
        DOCKER_TAG = 'latest'
    }
        ...
        stage('Deploy to app1') {
            steps {
                sshagent (credentials: ['app-ssh-key']) {
                    ...
                    docker rmi -f kh55s/jenkinstest || true
                    sleep 2
                    docker ps --filter "publish=8080" --format "{{.ID}}" | xargs -r docker rm -f
                    docker run -d --name app -p 8080:8080 kh55s/jenkinstest
ENDSSH'''
                }    
            }
        }
        ...
        stage('Deploy to app1') {
            steps {
                sshagent (credentials: ['app-ssh-key']) {
                    sh '''
                    ssh -o StrictHostKeyChecking=no ec2-user@172.31.0.188 <<'ENDSSH'
                    PID=$(lsof -ti:8080)
                    if [ ! -z "$PID" ]; then 
                        echo "Killing process using port 8080: $PID"
                        kill -9 "$PID"
